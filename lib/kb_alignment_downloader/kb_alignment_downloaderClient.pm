package kb_alignment_downloader::kb_alignment_downloaderClient;
...
kb_alignment_downloader::kb_alignment_downloaderClient
...
sub new
{
    ...
    my $self = {
	client => kb_alignment_downloader::kb_alignment_downloaderClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "kb_alignment_downloader.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "kb_alignment_downloader.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for kb_alignment_downloader::kb_alignment_downloaderClient\n";
    }
    if ($sMajor == 0) {
        warn "kb_alignment_downloader::kb_alignment_downloaderClient version is $svr_version. API subject to change.\n";
    }
}
...
package kb_alignment_downloader::kb_alignment_downloaderClient::RpcClient;
